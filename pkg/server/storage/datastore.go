...
import (
	...
	"github.com/elixirhealth/service-base/pkg/server/storage"
	"go.uber.org/zap"
...
func (d *datastoreStorer) Put(pr *api.PublicationReceipt) error {
	...
	err := d.client.Get(context.Background(), pubKey, &PublicationReceipt{})
	//cancel()
	...
	//defer cancel()
	if _, err = d.client.Put(context.Background(), pubKey, spr); err != nil {
		return err
	}
