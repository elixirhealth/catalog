...
PKG_SUBDIRS=$(shell go list ./... | grep -v /vendor/ | sed -r 's|github.com/elixirhealth/catalog/||g' | sort)
...
acceptance:
	...
	@mkdir -p artifacts
	@go test -tags acceptance -v github.com/elixirhealth/catalog/pkg/acceptance 2>&1 | tee artifacts/acceptance.log
...
build-static:
	...
	@./vendor/$(SERVICE_BASE_PKG)/scripts/build-static deploy/bin/catalog
...
docker-image:
	...
	@docker build --rm=false -t gcr.io/elxir-core-infra/catalog:snapshot deploy
...
proto:
	@echo "--> Running protoc"
	@protoc pkg/catalogapi/catalog.proto -I. -I vendor/ --go_out=plugins=grpc:.
